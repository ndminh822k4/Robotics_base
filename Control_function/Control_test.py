...
from Control_function.MPC import MPCController
...
M_start = M[:6, :6]
Q_pos = np.diag([1000, 3000, 2000, 500, 600, 550])
...
MPC = MPCController(
    M0=M,
    Q=Q,
    R=np.array([0.05, 0.005, 0.005, 0.005, 0.01, 0.05]),
    N=10,
    dt=0.01,
    torque_limit=None
)
...
with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
    ...
    while viewer.is_running():
        ...
        if controller_state["enabled"] and q_error_norm < 0.05 and dq_norm < 0.05:
            print("\n\n========================================")
