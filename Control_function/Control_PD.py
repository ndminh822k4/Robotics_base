...
# ============================================================
# 1. IMPORT FORWARD / INVERSE KINEMATICS / PID
# ============================================================
...
from Control_function.PID import PIDControl
...
# chính. Giao tiếp qua multiprocessing.Queue: process mô phỏng chỉ "ném" các
# con số nhỏ (không phải mảng lớn/model MuJoCo) vào queue, process vẽ tự đọc
# và tự update đồ thị theo tốc độ của riêng nó.
#
# LƯU Ý: dùng start method "fork" (chỉ có trên Linux/macOS) để không phải
# tái import và chạy lại toàn bộ script (IK, load model...) trong process
# con như cách "spawn" yêu cầu. Trên Windows sẽ cần tái cấu trúc lại script
# với if __name__ == "__main__" và "spawn".
def live_plot_worker(data_queue, torque_limit, max_points=3000, refresh_dt=0.05):
    ...
    fig.suptitle("Live monitor (process riêng)")
...
# ============================================================
def gravity_compensation_torque(model, data_tmp, q):
    data_tmp.qpos[:6] = q
    data_tmp.qvel[:6] = 0.0
    mujoco.mj_forward(model, data_tmp)
    data_tmp.qacc[:6] = 0.0
    mujoco.mj_inverse(model, data_tmp)
    return data_tmp.qfrc_inverse[:6].copy()
...
# 9. PID PARAMETERS
# ============================================================
Kp = np.array([1000, 1000, 800, 500, 300, 200], dtype=float)
Kd = np.array([80, 80, 60, 30, 15, 8], dtype=float)
Ki = np.array([0, 0, 0, 0, 0, 0], dtype=float)  # bật lại, có anti-windup bên dưới
...
# ============================================================
# 14. DATA PHỤ CHO GRAVITY COMPENSATION
# ============================================================
gcomp_data = mujoco.MjData(model)

# ============================================================
# 14b. GRAVITY & CONTROLLER TOGGLE (bật/tắt bằng phím tắt)
# ============================================================
# Lưu trọng lực gốc của model để bật lại đúng giá trị khi cần
...
GRAVITY_TOGGLE_KEY = " "     # Space: bật/tắt trọng lực (của MÔ PHỎNG MuJoCo)
CONTROLLER_TOGGLE_KEY = "c"  # C: bật/tắt bộ điều khiển PID (bao gồm cả tau_g bù trọng lực)
...
def set_controller(enabled: bool):
    if enabled:
        print("\n[INFO] Bộ điều khiển PID: BẬT — bắt đầu tính mô-men (PID + bù trọng lực) để bám điểm đích.")
    else:
        print("\n[INFO] Bộ điều khiển PID: TẮT — mô-men = 0, robot rơi/đứng tự do theo vật lý.")
    controller_state["enabled"] = enabled


def key_callback(keycode):
    """Callback phím cho mujoco.viewer.launch_passive.
    Space: bật/tắt trọng lực của mô phỏng (model.opt.gravity).
    C    : bật/tắt bộ điều khiển PID (kể cả phần bù trọng lực tau_g).
    Lưu ý: nếu bạn bật trọng lực nhưng TẮT bộ điều khiển, robot sẽ rơi hoàn
    toàn tự do (không có gravity compensation) vì tau_g chỉ được cộng khi
    controller đang BẬT.
    """
    try:
...
print("  - Nhấn C     : bật/tắt BỘ ĐIỀU KHIỂN PID (mặc định đang TẮT).")
print("  Quy trình test gợi ý: để mọi thứ TẮT -> robot đứng yên -> bật trọng")
print("  lực (SPACE) -> robot RƠI tự do -> bật bộ điều khiển (C) -> PID + bù")
print("  trọng lực sẽ tự tính mô-men để kéo robot từ vị trí đang rơi về đích.")
...
with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
    ...
    while viewer.is_running():
        ...
        velocity_error = -dq_current

        if controller_state["enabled"]:
            tau_g = gravity_compensation_torque(model, gcomp_data, q_current)

            tau_pid, integral_error = PIDControl(
                q_current=q_current, q_desired=q_final,
                dq_desired=np.zeros(6), dq_current=dq_current,
                integral_error=integral_error, dt=dt_sim,
                kp=Kp, ki=Ki, kd=Kd, torque_limit=torque_limit
            )

            # Anti-windup: giới hạn tích phân sai số để không tích lũy vô hạn khi
            # torque bị bão hòa lâu, tránh overshoot mạnh khi thoát bão hòa
            integral_error_max = torque_limit / np.maximum(Ki, 1e-6)
            integral_error = np.clip(integral_error, -integral_error_max, integral_error_max)

            torque = tau_pid + tau_g
            # Dùng torque_limit đã tính đúng (có xét forcelimited), KHÔNG dùng
            # trực tiếp forcerange thô vì có thể là placeholder [0,0]
            torque = np.clip(torque, -torque_limit, torque_limit)
            data.ctrl[:6] = torque
        else:
            # Bộ điều khiển đang TẮT -> không sinh mô-men (kể cả bù trọng lực)
            # -> robot rơi/đứng tự do hoàn toàn theo vật lý.
            integral_error = np.zeros(6)
            data.ctrl[:6] = 0.0
        ...
        viewer.sync()
        time.sleep(0.00001)  # sleep 10ms để giảm tải CPU, không cần thiết nhưng giúp máy mát hơn
...
def plot_time_response(log_t, log_q_error_norm, log_q_error, log_dq_norm,
    ...
    def shade_events(ax):
        # Tô nền để biết đoạn nào trọng lực/bộ điều khiển đang bật
    ...
    fig.suptitle("Đáp ứng thời gian của bộ điều khiển", fontsize=14)
...
plot_save_path = os.path.join(CONTROL_DIR, "lqr_time_response.png")
plot_time_response(
    log_t, log_q_error_norm, log_q_error, log_dq_norm, log_dq,
    log_torque, log_ee_pos, log_gravity_on, log_controller_on,
    torque_limit, metrics_t0=metrics_t0, metrics=metrics, save_path=plot_save_path
)
