...
from Control_function.LQR import LQRControl, compute_LQR_gain
...
def live_plot_worker(data_queue, torque_limit, max_points=3000, refresh_dt=0.05):
    ...
    while True:
        ...
        stop_signal = False
        try:
            # Rút hết các item đang chờ trong queue, chỉ giữ lại cái mới nhất
            # nếu process vẽ bị chậm hơn tốc độ mô phỏng đẩy dữ liệu vào.
            while True:
        ...
        if item is None:
            # Không có dữ liệu mới -> vẫn phải "bơm" event loop GUI để cửa sổ
            # không bị đơ/"not responding"
...
Q_lqr = np.diag([800, 800, 600, 400, 200, 150] + [40, 40, 30, 20, 10, 8])
R_lqr = np.eye(6) * 0.05
...
# ============================================================
# 9b. GRAVITY TOGGLE (bật/tắt trọng lực bằng phím tắt)
# ============================================================
# Lưu lại vector trọng lực gốc của model để có thể bật lại đúng giá trị
gravity_original = model.opt.gravity.copy()

# Trạng thái trọng lực: ban đầu TẮT (robot đứng yên, không rơi)
gravity_state = {"enabled": False}
model.opt.gravity[:] = 0.0

GRAVITY_TOGGLE_KEY = " "  # phím Space để bật/tắt trọng lực
CONTROLLER_TOGGLE_KEY = "c"  # phím C để bật/tắt bộ điều khiển LQR

# Trạng thái bộ điều khiển: ban đầu TẮT (robot đứng yên tự do, chưa điều khiển)
...
def set_controller(enabled: bool):
    if enabled:
        print("\n[INFO] Bộ điều khiển LQR: BẬT — bắt đầu tính mô-men để bám điểm đích.")
    else:
        print("\n[INFO] Bộ điều khiển LQR: TẮT — mô-men = 0, robot rơi/đứng tự do theo vật lý.")
...
print("  - Nhấn C     : bật/tắt BỘ ĐIỀU KHIỂN LQR (mặc định đang TẮT).")
print("  Quy trình test gợi ý: để mọi thứ TẮT -> robot đứng yên (không trọng")
print("  lực, không điều khiển) -> bật trọng lực (SPACE) -> robot RƠI tự do")
print("  -> bật bộ điều khiển (C) -> LQR sẽ tự tính mô-men bù để kéo robot")
print("  từ vị trí đang rơi về bám theo quỹ đạo tới điểm đích.")
...
with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
    ...
    while viewer.is_running():
        ...
        if controller_state["enabled"]:
            torque, K = LQRControl(
                q_desired=q_final, q_current=q_current,
                dq_desired=np.zeros(6), dq_current=dq_current,
                M=M, bias=bias, Q=Q_lqr, R=R_lqr, qdd_desired=None,
        ...
        # Chỉ đẩy mỗi LIVE_PLOT_EVERY bước, và dùng put_nowait để KHÔNG BAO
        # GIỜ chặn vòng lặp mô phỏng chính nếu process vẽ đang xử lý chậm.
        step_counter += 1
