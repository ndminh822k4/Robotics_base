...
# ============================================================
# 11c. THAY ĐỘNG LỰC HỌC DỰ BÁO BẰNG MẠNG HỌC MÁY (đã train trên Colab)
# ============================================================
# Mặc định NMPCController dùng make_mujoco_dynamics_func() (gọi
# mj_forward + mj_fullM ở MỖI bước dự báo trong vòng lặp L-BFGS-B ->
# đúng nhưng chậm). Ở đây thay bằng dynamics_func học từ dữ liệu
# (dyn_model.npz, train trên Colab) — CHỈ CẦN 1 DÒNG, vì compute()
# chỉ gọi self.dynamics_func(q, dq, u), không quan tâm bên trong là
# MuJoCo hay mạng nơ-ron.
#
...
if USE_LEARNED_DYNAMICS:
    if os.path.exists(DYN_MODEL_PATH):
        MPC.dynamics_func = load_learned_dynamics(DYN_MODEL_PATH)
        print(f"\n[INFO] NMPC đang dùng ĐỘNG LỰC HỌC ĐÃ HỌC ({DYN_MODEL_PATH}) "
              f"thay cho MuJoCo thật trong bước dự báo.")
    else:
...
with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
    ...
    while viewer.is_running():
        ...
        if controller_state["enabled"] and q_error_norm < 0.03 and dq_norm < 0.05:
            print("\n\n========================================")
